...
def get_fangraphs_data(x, y):
    ...
    if y == 'Batter':
        ...
        with open('colkeys_fangraphs_batter.txt', 'w') as f:

            # change the standard output to the file created
            sys.stdout = f

            # print column index and name to file
            print(list(enumerate(stats.columns)))

            # reset the standard output to its original output
        ...
        with open('colkeys_fangraphs_pitcher.txt', 'w') as f:
        ...
        # use certain metric columns only
        df = stats.iloc[:, np.r_[0:6, 8, 9, 13, 15, 16, 26, 40, 43:46, 47, 63, 111, 114, 323:332, 333, 334]].reset_index(drop=True)
